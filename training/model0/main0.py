...
import src.config_0 as config
import pandas as pd
from train_dqn_modular_0 import run_train_dqn_both_timesteps
import numpy as np
...
def save_experiment_parameters(save_results_big_run, MAX_TOTAL_TIMESTEPS):
    ...
    try:
        ...
        train_file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'train_dqn_modular_0.py')
    ...
    # Dynamically read reward parameters from config_0.py
    reward_params = {}
    try:
        config_file_path = os.path.join(PROJECT_ROOT, 'src', 'config_0.py')
...
if __name__ == "__main__":
    ...
    # Baseline (model0) results path
    save_folder = "results/model0/training/m0_baseline"
