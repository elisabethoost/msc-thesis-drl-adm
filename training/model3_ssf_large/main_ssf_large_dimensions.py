...
import pandas as pd
from train_dqn_modular_ssf_large_dimensions import run_train_dqn_both_timesteps
import numpy as np
...
def save_experiment_parameters(save_results_big_run, MAX_TOTAL_TIMESTEPS):
    ...
    # Dynamically read training parameters from train_dqn_modular_ssf_large_dimensions.py
    training_params = {}
    try:
        # Read the file and extract parameter values using regex
        train_file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'train_dqn_modular_ssf_large_dimensions.py')
    ...
    reward_params = {}
    try:
        config_file_path = os.path.join(PROJECT_ROOT, 'src', 'config_ssf.py')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--env_type", type=str, choices=['myopic', 'proacmaxtive', 'reactive', 'drl-greedy'], help="Environment type to run")
    ...
    save_folder = "results/model3_ssf_large/training/m3_2"
    ...
    if args.seed is None and args.training_folder is None:
        ...
        processes = []
        # Get the path to the virtual environment's Python interpreter (in project root)
        ...
        for seed in SEEDS:
            for env_type in env_types:
                ...
                env = os.environ.copy()
                env["PYTHONPATH"] = PROJECT_ROOT  # Set to project root so imports work
