...
def run_train_dqn_both_timesteps(
    ...
    def train_dqn_agent(env_type, seed, save_training_monitor=False):
        ...
        # Initialize training monitor only if enabled
        monitor = None
        if save_training_monitor:
        ...
        if save_training_monitor and monitor:
            ...
            print("Training monitor disabled - starting fresh training")
            total_timesteps = 0
            episode_start = 0
            rewards = {}
            test_rewards = []
        ...
        while total_timesteps < MAX_TOTAL_TIMESTEPS:
            ...
            for scenario_folder in scenario_folders:
                ...
                while not done_flag and not max_steps_reached:
                    ...
                    # Monitor system health every 10,000 timesteps (only if monitor enabled)
                    if save_training_monitor and monitor and total_timesteps % 10000 == 0:
                        monitor.check_system_health(
                            model, total_timesteps, episode, rewards, test_rewards, epsilon_values, epsilon
                        )

                    # Add checkpoint saving every 50,000 timesteps (only if monitor enabled)
                    if save_training_monitor and monitor and total_timesteps % 50000 == 0:
    ...
    rewards, test_rewards, total_timesteps, epsilon_values, good_rewards, action_sequences, model_path, detailed_episode_data = train_dqn_agent(env_type, single_seed, save_training_monitor)
